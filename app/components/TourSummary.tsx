...
export default function TourSummary() {
  ...
  return (
    <section
      ref={sectionRef}
      id="about"
      className="py-24 md:py-32 bg-main-bg relative overflow-hidden"
    >
      {/* Decorative background elements */}
      <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-accent-cta/5 rounded-full blur-[100px] pointer-events-none" />
        ...
        >
          <div className="flex flex-col lg:flex-row gap-16 items-start">
            ...
            <div className="lg:w-3/5">
              ...
              <p className="text-2xl font-medium text-dark-section/80 mb-6 italic">
                Мы отправляемся в новое путешествие с Tsunami Travel!
              </p>

              <div className="space-y-6 text-xl text-dark-section/70 leading-relaxed font-light">
                <p>
                ...
                <p>
                  ...
                  <span className="font-bold text-dark-section">
                    Будет сказочно!
                  </span>
                </p>
              </div>
            ...
            <div className="lg:w-2/5 w-full space-y-6">
              {details.map((item, idx) => (
                <div
                  key={idx}
                  className={`bg-light-surface/50 backdrop-blur-sm border border-dark-section/10 p-8 rounded-3xl transition-all duration-500 hover:border-accent-cta/30 hover:shadow-xl group delay-[${
                    idx * 150
                  }ms]`}
                >
                  <div className="flex gap-6 items-center">
                    <div className="w-16 h-16 rounded-2xl bg-accent-cta/10 flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                      {item.icon}
                    </div>
                    <div>
                      <p className="text-sm uppercase tracking-widest text-dark-section/50 font-bold mb-1">
                        {item.label}
                      </p>
                      <p className="text-2xl font-bold text-dark-section">
                        {item.value}
                      </p>
                      <p className="text-sm text-dark-section/60">{item.sub}</p>
                    </div>
                  </div>
                </div>
              ))}
              ...
              {/* Special CTA Card */}
              <div className="bg-dark-section p-8 rounded-3xl text-light-surface overflow-hidden relative group">
                <div className="relative z-10">
                  <h4 className="text-xl font-bold mb-2">Готовы к поездке?</h4>
                  <p className="text-light-surface/60 mb-6">
                    Забронируйте свое место в группе прямо сейчас. Количество
                    мест ограничено.
                  </p>
                  <button className="w-full bg-accent-cta hover:bg-accent-cta/90 text-white font-bold py-4 rounded-xl transition-all transform hover:scale-[1.02]">
                    Забронировать тур
                  </button>
                </div>
                <div className="absolute top-0 right-0 -translate-y-1/2 translate-x-1/2 w-40 h-40 bg-accent-cta/20 rounded-full blur-3xl opacity-0 group-hover:opacity-100 transition-opacity" />
              </div>
            </div>
          </div>
        </div>
